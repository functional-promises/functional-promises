...
FP.prototype.catch = function catchProxy(fn: unknown) {
  if (this.steps) return this.addStep('catch', Array.from(arguments))
  if (arguments.length === 2) return this.catchIf(...Array.from(arguments))
  if (!isFunction(fn)) {
...
function promisify(cb: (...args: unknown[]) => unknown) {
  return (...args: unknown[]) =>
    new (FP as any)((yah: (value: unknown) => void, nah: (error: unknown) => void) =>
      cb.call(this, ...args, (err: unknown, res: unknown) => (err ? nah(err) : yah(res))))
}
...
function reject(this: any, err: Error) {
